...
ダイクストラのアルゴリズムを実装した`dijkstra`を作成した。

本当の難しさはプログラミングそのものではなく、アルゴリズムの設計にある。  
解法を自分で考える際の手助けになるのが、デザインレシピ。

## 16.5　プログラム作成のプロセス

プログラムで最も大切なのは、正しく動作すること。  
だからまずは、正しく動くプログラムを作る。

しかし、出来上がったプログラムの効率に問題があることがある。  
そのときは、非効率な箇所をひとつずつ改善してプログラムを改良していく。  
例えば`16.4`で作成した`dijkstra`は、速度に問題があり、改善の余地がある。

プログラムの改良を行いやすくしておくためにも、機能単位に分割して作るべき。
